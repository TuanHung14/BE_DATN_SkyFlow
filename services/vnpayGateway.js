...
exports.createVNPayPayment = async (paymentData, req) => {
    try {
        const { orderId, amount, orderInfo = `Thanh-toan-don-hang-${orderId}` } = paymentData;
        const vnp_TmnCode = process.env.VNPAY_TMN_CODE;
        const vnp_HashSecret = process.env.VNPAY_SECRET_KEY;
        const vnp_Url = process.env.VNPAY_API_ENDPOINT || 'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html';
        const returnUrl = process.env.VNPAY_RETURN_URL || 'http://localhost:3000/vnpay/callback';
...
exports.verifyVNPayCallback = (callbackData) => {
    try {
        ...
        const requiredFields = ['vnp_TxnRef', 'vnp_Amount', 'vnp_ResponseCode', 'vnp_TransactionStatus', 'vnp_SecureHash'];
        ...
        throw new AppError(`Không thể xác minh callback VNPay: ${error.message}`, 400);
    }
};